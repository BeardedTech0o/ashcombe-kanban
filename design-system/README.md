# getit / Brainbox design system, v1
...
- Font family: system default for now, no display face chosen yet
...
- Typeface. Still on system font stack, no display or body face picked.
...
1. **Audit before touching anything.** Get Claude Code to grep both codebases (getit and Brainbox PWAs, plus the EXE) for hardcoded colours, border radius values, box shadow declarations, and spacing (padding/margin) values. That list is the actual scope, not a guess.
...
4. **getit specifically**: since it's mid migration to the Cloudflare stack, apply tokens as part of that rebuild rather than to the Supabase version you're retiring. No point styling code you're about to replace.
5. **Brainbox specifically**: this one's already built and iterated on, so treat it as the higher risk retrofit. Take screenshots of key screens before starting, so you've got a clear before and after to catch regressions.
6. **EXE build**: confirm whether it shares a webview/HTML rendering layer with the PWAs or is native. If it renders HTML, `tokens.css` drops straight in. If it's native UI, `tokens.json` is still the source of truth, but someone (or Claude Code) needs to translate it into whatever the native styling system uses, since CSS custom properties won't apply directly.
7. **Watch for the gaps.** Anywhere the retrofit hits dark mode, a typeface decision, or a colour outside the teal accent (success/error states, for instance), stop and flag it rather than improvising a value that isn't in `tokens.json`. Improvised one off values are exactly how design systems drift.
...
2. Build a small shared package or file that both getit and the EXE builds can import
3. Run the audit in step 1 above first, and share the results before writing any component changes
4. Apply the tokens to one real screen first (suggest the getit task list) rather than the whole app at once, so you can react to it in context before rolling it out everywhere
